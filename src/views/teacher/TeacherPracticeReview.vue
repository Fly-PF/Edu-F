...
async function openAiReview() {
  ...
  const firstOpenAnswer = activeSubmission.value.answers.find((answer) => answer.questionType === 'SHORT')
  reviewVisible.value = false
  await router.push({
    name: 'teacher-ai-grading',
    query: {
      submissionId: String(activeSubmission.value.submissionId),
      questionId: String(firstOpenAnswer.questionId),
...
<template>
  <main class="practice-review-page">
    ...
    <section class="review-toolbar"><el-radio-group v-model="status" @change="loadSubmissions"><el-radio-button value="SUBMITTED">待批改</el-radio-button><el-radio-button value="REVIEWED">已批改</el-radio-button><el-radio-button value="">全部</el-radio-button></el-radio-group></section>
    ...
    <el-dialog v-model="publishVisible" class="publish-dialog" width="min(920px, 94vw)" title="发布习题" destroy-on-close>
      <el-form label-position="top" class="publish-form">
        ...
        <article v-for="(question, index) in publishForm.questions" :key="index" class="publish-question">
          ...
          <template v-if="question.type === 'SINGLE'"><div class="options-title"><span>选项（固定 A、B、C、D）</span><el-button text type="primary" :disabled="question.options.length >= 4" @click="addOption(question)">添加选项</el-button></div><div v-for="(option, optionIndex) in question.options" :key="optionIndex" class="option-row"><b>{{ String.fromCharCode(65 + optionIndex) }}</b><el-input v-model="question.options[optionIndex]" :placeholder="`选项 ${String.fromCharCode(65 + optionIndex)}`" /><el-button text type="danger" :disabled="question.options.length <= 4" @click="removeOption(question, optionIndex)">删除</el-button></div><el-form-item label="正确答案"><el-radio-group v-model="question.referenceAnswer"><el-radio v-for="(_, optionIndex) in question.options" :key="optionIndex" :value="String.fromCharCode(65 + optionIndex)">{{ String.fromCharCode(65 + optionIndex) }}</el-radio></el-radio-group></el-form-item></template>
